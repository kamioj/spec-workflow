...
| check-archive | archive & ship | flow was bypassed (no APPROVED / unchecked tasks / no proposal); **loop/fix/quick changes are audited differently** (loop.md status: done + checked Acceptance; fix.md status: shipped + non-empty Audit; legacy quick.md status: done + non-empty Evidence — no proposal.md exists by design); **ship invocations get a precondition-only audit** (fixes batch exists with ≥1 entry — never the shipped status ship itself writes); override = prompt contains `force` or `abandon(ed)` — **matched against the prompt value only**, never the whole stdin JSON (a cwd containing "force" must not bypass; that was finding V-1) |
...
│   ├── fix.md       at-fix    streaming light-tier batch ledger, only in changes/fixes/ (fix.md without proposal.md = light-tier dir, not counted active; legacy quick.md dirs keep the same treatment)
