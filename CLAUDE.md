...
这是一个 **Claude Code plugin marketplace**，里面装着 `spec`（spec-workflow / sdd）插件。**没有应用代码、没有编译/构建/测试运行器**——"源码"就是 markdown 命令、JSON 清单、hook 脚本。修改即生效，"测试"靠加载插件并实跑命令。
...
**关键：改了 `hooks/` 下任何东西（hooks.json / .sh / .py / .ps1）必须重启 Claude 才生效**——commands / skills / agents 是热加载，hook 不是。这是最容易踩的坑：改完 hook 不重启，测出来的是旧行为。
...
**macOS/Linux 优先**：hook 默认通过 `sh` 入口调用 `python3` 解析 Claude Code 的 JSON stdin；找不到 `python3` 时会阻断并提示安装方式，也可运行 `sh hooks/install-python3.sh`。Windows 保留 `.ps1` 备用脚本，运行时用 `pwsh` 不用 `powershell`，避免 PS 5.1 默认 GBK 编码弄乱码。
...
| Hook | 匹配 | 拦什么 | 不满足时 |
|---|---|---|---|
| `check-tbd.sh` / `check-tbd.ps1` | `/spec:propose` | research.md 的 `## Open [TBD]` 段还有 `[TBD-N]` | `exit 2`，提示走 `/spec:ask` |
| `check-gate.sh` / `check-gate.ps1` | `/spec:apply` | 缺活跃 change / proposal.md | `exit 2`，提示先走 `/spec:research` / `/spec:propose` |
...
- `check-gate` 不检查 APPROVED；`/spec:apply` 会在进入实施时自动追加 APPROVED 标记。不要把“apply 前必须已 APPROVED”的旧逻辑加回来。

## 架构大图：命令 + agent + 产物

**12 个独立 slash 命令**（`commands/*.md`），每个可单独触发、可重入、可单点重做——这是相对 OpenSpec（4 命令一把梭）/ superpowers（9 步硬流程）的定位差异。典型流：
`research → ask → (design) → propose → [HARD GATE] → apply → verify → handoff → archive`。`/spec:workflow` 是全流程一把梭，`/spec:status` 报告当前在哪一步，`/spec:handoff` 用于新会话接续和 context 瘦身。
...
**2 个开发 agent**（`agents/spec-{frontend,backend}-dev.md`），在 `/spec:apply` 阶段派遣：
