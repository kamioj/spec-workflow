...
**16 independent commands** (`core/commands/*.md` → both hosts), each re-entrant and re-runnable — the positioning difference from OpenSpec (4 commands all-in-one) / superpowers (rigid 9 steps). Typical flow: `research → ask → (design) → propose → [HARD GATE] → apply → verify → archive`; `workflow` runs end-to-end, `status` reports position, `loop` runs goal-driven autonomous rounds (two touchpoints, Stop-driver enforced), `fix` is the streaming light tier (F-N entries appended to the standing `spec/changes/fixes/` dir; size advisory, never a refusal), `ship` closes the fix batch (ONE verifier audit over the accumulated diff, then archives the dir; `<date>-fixes` counter-suffixed on same-day collision), `stash`/`resume` suspend and revive a change via a `.paused` marker. **Active-change counting rule (all three prompt gates + the Stop reminder)**: dirs carrying `.paused`, or `quick.md`/`fix.md` without proposal.md, don't count — precedence is "proposal.md presence wins" (an upgraded light-tier dir is a normal full change); check-archive audits fix batches by fix.md `status: shipped` + non-empty Audit and legacy quick dirs by quick.md `status: done` + non-empty Evidence (loop-branch trust model), and gates `/spec:ship` invocations on **preconditions only** (batch exists with ≥1 F-N entry — never the shipped status ship itself writes; the check-gate/apply deadlock lesson applied).
