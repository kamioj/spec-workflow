#!/usr/bin/env pwsh
# /sdd:propose 前置检查：research.md 的 ## Open [TBD] 必须清空
# 触发：UserPromptSubmit hook
# 行为：用户输入含 /sdd:propose 时扫描 research.md；含 [TBD] 则 exit 2 阻断
...
try {
    ...
    # 只在用户输入含 /sdd:propose 时触发
    if ($userPrompt -notmatch '/sdd:propose') { exit 0 }
    ...
    if (-not $changes -or $changes.Count -eq 0) {
        [Console]::Error.WriteLine('SDD: 没有活跃 change。先调 /sdd:research <方向> 启动')
        exit 2
    }
    ...
    foreach ($change in $changes) {
        ...
        if (-not (Test-Path $researchPath)) {
            [Console]::Error.WriteLine("SDD: $($change.Name) 缺 research.md。先调 /sdd:research <方向>")
            exit 2
        }
        ...
        if ($content -match '(?ms)^##\s*Open\s*\[TBD\][\s\S]*?(?=^##\s+|\z)') {
            ...
            if ($openSection -match '\[TBD-\d+\]') {
                [Console]::Error.WriteLine("SDD: research.md ($($change.Name)) 含未消化的 [TBD] 决策点。先调 /sdd:ask 消化")
                exit 2
            }
        }
    }
