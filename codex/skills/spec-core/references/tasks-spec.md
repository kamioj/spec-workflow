...
Produced by `$spec-propose` (trigger conditions + generation steps are detailed in the propose command § "When to also generate tasks.md"). **The generation decision is declared at the HARD GATE** (which trigger fired + how the work was split) — never silently attached.
