...
1. **Confirm the change directory**: no active change → create `spec/changes/<kebab-name>/`, deriving the name from the user's description, **and write the current pointer** (`spec/changes/.current` ← the new name — gates target it even when other changes coexist); active change with a matching direction → append to research.md. **Old-change triage (one question round, then proceed)**: creating a new change while OTHER active changes exist → before diving into research, surface them in ONE structured question (delivered per $spec-ask's question rules — structured tool first, plain-text fallback) and let the user disposition each — a change whose verify.md concludes pass gets **archive (recommended)**; an unfinished one gets **stash** or **keep parallel** (legal under the pointer — the new change is already targeted). Never present this as an error: the new change proceeds regardless of the answer; the question exists so forgotten changes get closed instead of accumulating. Chosen archives/stashes execute right after the question, before step 2. **spec/ is created at the project root the session was launched from — NEVER inside a subproject directory, and NEVER in another spec tree**: the gate hooks resolve spec/ at that root only, and a spec tree anywhere else is invisible to them (apply/ship/archive would block with "no spec/changes/ directory"). Working on a monorepo subproject → keep spec/ at the launch root (prefix the change name with the subproject if helpful), or have the user relaunch the session inside the subproject. A git worktree session roots spec/ in THAT worktree — do not follow `.git`/gitdir back to the main repository. **Path self-check (mandatory)**: right after creating the directory, confirm its absolute path is `<launch root>/spec/changes/<name>/` — a change created in any other tree makes every later gate block with misleading messages (the gate names whatever unrelated change its own tree holds).
...
4. Main conversation maps the status quo (**always — this step never gets skipped**) — **consult the knowledge base first, two-step**: scan `spec/knowledge.md` (the index) and open only the subdocs whose tag+hook match this change's domain (legacy flat file → read whole, declare; format → references/knowledge-spec.md); don't re-derive or re-Grep what's already recorded there — then Grep / Glob relevant modules to **map existing call chains / constraints** (write into `## Constraints` — this is "understanding the status quo", not "designing new architecture"; new architecture belongs in design). A knowledge.md fact contradicted by what you find → note the correction in research (`订正/corrected: ...`); the fix to knowledge.md itself lands at archive time.
5. **Build `spec/changes/<name>/index.md`** (mandatory for new changes; format → references/index-spec.md) at **first contact** with the requirement source: extract R-N **verbatim quotes** of behavioral clauses only (validation / required fields / value ranges / permissions / display mappings / state rules — descriptive narrative is not indexed); from the status-quo mapping, inventory the A-N assets this change's domain touches (`use: reuse / extend / pattern / rejected: <reason>`) and designate E-N exemplars for each new page/module (1–2 existing ones as the master template). **Two hard rules ride the A-N inventory**: ① an A-N row marked `extend` or `modify` MUST carry an **impact enumeration** — the callers / consumers / live-data constraints of the touched surface (Grep-evidenced), landed in `## Constraints`; this enumeration is what lets the manual flow run without a compat critic — skip it and the change's blast radius surfaces only at delivery. ② every **planned new artifact** (entity / utility / component / dependency) gets a **project-wide equivalent search** (not just this change's domain — an equivalent living in another module still counts), result recorded on its A-N row: found → reuse candidate; empty-handed → the search scope recorded (the reuse lens verifies these claims later with its own codebase access); **map the C-N Carriers** from the same status-quo pass — every requirement noun that will need a data/API carrier (field / method / endpoint / parameter) gets a row mapping it to its EXISTING carrier (e.g. 单据类型 → `is_device`); no existing carrier found → record the responsibility-search (where you looked) and mark the row `minting required` (it rides the HARD GATE for approval). Requirement nouns are not entitled to new code entities — the map is what stops 1:1 noun-to-entity minting. **One extraction** — propose freezes the index; later stages append new IDs only, never renumber. No prototype/document → the user's task statement IS the source: quote its sentences (a few lines; mandatory does not mean heavy).
...
8. **Close with the next command, keyed to `## Open` AND the gate pre-flight**: end the research turn by reporting the Open [TBD] state and the matching next step — `## Open` holds [TBD-N] items → the next command is `$spec-ask` (name the open TBD-N ids so the user knows what will be asked); `## Open` empty → `$spec-propose` **only after the target pre-flight passes** (SKILL § Gate-aware next steps: current pointer set or exactly one active — research writes the pointer at dir creation, so this normally holds; ambiguous anyway → recommend `$spec-resume <this change>` and list the other actives with archive candidates flagged). **NEVER recommend a command whose gate you can see will bounce it** — a wrong recommendation sends the user straight into the wall.
...
Load the corresponding reference only when writing Practices that involve a specific tech stack (all files below live under `../spec-core/references/`):
- Java + Spring → `../spec-core/references/alibaba-java.md` + `java-conventions.md`
