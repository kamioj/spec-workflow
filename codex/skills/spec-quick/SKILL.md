...
1. Create `spec/changes/<kebab-name>/quick.md` (format → [`../spec-core/references/quick-spec.md`](../spec-core/references/quick-spec.md)): `status: in-flight`, `## Ask` = one-line restatement + the user's words **quoted verbatim** (the mini anchor — interpretation happens against the quote)
2. Implement directly. The Coding Charter binds (Read `../spec-core/references/code-charter.md` before the first keystroke — same rule as $spec-apply); the Concerns discipline binds (requirement silent → most permissive behavior; tightening impulses go to `## Concerns`, never into code; a blocking structural decision → stop and ask immediately)
