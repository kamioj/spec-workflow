...
|  | `/sdd:propose [--codex]` | 写 proposal.md；--codex 让 codex 挑刺方案 |
|  | `/sdd:revise [section]` | 局部改 proposal（why/what/how/risk） |
| 执行 & 验证 | `/sdd:apply` | 实施代码 |
|  | `/sdd:verify [--codex] [--fix]` | 自审三维；--codex 加 codex 他审，--fix 让 codex 改 |
| 收尾 | `/sdd:archive` | 归档 |
...
hook `check-gate.ps1` 在 `/sdd:apply` 执行前检查此标记。**无标记 → 拒绝执行**。
...
| Hook 脚本 | 触发命令 | 作用 |
|---|---|---|
| `hooks/check-tbd.ps1` | `/sdd:propose` 前 | research.md 含 `[TBD]` 则拒绝执行，提示走 `/sdd:ask` |
| `hooks/check-gate.ps1` | `/sdd:apply` 前 | proposal.md 缺 `APPROVED` 标记则拒绝执行 |
...
按需读，**不强制**：
- `skills/workflow/references/alibaba-java.md` + `java-conventions.md` — Java + Spring
- `skills/workflow/references/vue-style.md` + `vue-patterns.md` + `js-style.md` + `css-style.md` — Vue（uni-app 加 `uniapp-miniprogram.md`）
- `skills/workflow/references/bulletproof-react.md` + `react-patterns.md` — React
- `skills/workflow/references/google-ts-style.md` + `ts-conventions.md` — TS（叠加于 Vue/React/Node 之上）
- `skills/workflow/references/python-conventions.md` — Python
- `skills/workflow/references/php-conventions.md` — PHP
- `skills/workflow/references/flutter-conventions.md` — Flutter / Dart
