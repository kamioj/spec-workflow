...
`/spec:apply` 调 `scripts/mint-fingerprint.ps1` 铸契约指纹（写 `.fingerprint.json` + 在标记里带 `fp:sha256:<combined>`）。`check-source-gate.ps1`（PreToolUse 动作级门）在写 `spec/` 外源码时检查：**无 APPROVED 或契约指纹漂移 → 拒绝写源码**。
...
| Hook 脚本 | 事件 / 触发 | 作用 |
|---|---|---|
| `hooks/check-tbd.ps1` | UserPromptSubmit · `/spec:propose` | research.md 含 `[TBD]` 则拒绝，提示走 `/spec:ask` |
| `hooks/check-source-gate.ps1` | PreToolUse · `Write\|Edit\|MultiEdit` | 写 `spec/` 外源码但活跃 change 未 APPROVED 或契约指纹漂移 → 拒绝（决策→实施 动作级门，一把梭绕不过） |

**软约束 vs 硬约束**：
- 软约束（prompt）：模型可能违反，违反率取决于模型水平
- 硬约束（hook）：shell 脚本拦截，违反率 0

`hooks/` 下的 PowerShell 脚本由 `hooks/hooks.json` 注册到 `UserPromptSubmit`（check-tbd）和 `PreToolUse`（check-source-gate）事件。
