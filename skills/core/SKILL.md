---
name: sdd
description: Spec-driven development 工作流总览。当用户说"先 spec / 先出方案 / 先设计 / 提案"或任务规模 >150 行 / 跨 3+ 文件 / 引入新依赖 / 涉及架构选择时，加载此 skill 了解 spec-workflow/sdd plugin 的 12 个命令、产物地图、共享精神（HARD GATE / 拷问 / 卡死保护 / 反作弊 / context hygiene / 微任务委派）
---
...
`/spec:apply` 会在进入实施时追加此标记；hook `check-gate.sh` / `check-gate.ps1` 在 `/spec:apply` 执行前检查 change / proposal 是否存在。
...
| Hook 脚本 | 触发命令 | 作用 |
|---|---|---|
| `hooks/check-tbd.sh` / `hooks/check-tbd.ps1` | `/spec:propose` 前 | research.md 含 `[TBD]` 则拒绝执行，提示走 `/spec:ask` |
| `hooks/check-gate.sh` / `hooks/check-gate.ps1` | `/spec:apply` 前 | 缺活跃 change / proposal.md 则拒绝执行 |
