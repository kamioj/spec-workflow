...
6. **Native** (opt-in — run ONLY when the dispatch explicitly requests it, i.e. the user
   invoked `/spec:verify native`): project-idiom conformance. For EACH touched file, build
   its reference set — the designated E-N exemplar when index.md names one, otherwise 1–2
   nearest same-type files in THIS project (same directory / same layer) — and compare the
   change against how this project actually writes that kind of code: naming, component
   shape & props conventions, styling approach (whatever the project uses — CSS modules /
   utility classes / inline — not what a style guide prefers), state-management idiom,
   error-handling shape, API-call pattern. A deviation is a finding ONLY with **dual
   citation**: the project convention at file:line (≥2 existing occurrences — a single
   occurrence is not a convention, skip it) vs this change's deviation at file:line.
   **The project's own code is the sole authority here**: a deviation toward an objectively
   cleaner pattern is still a finding — consistency outranks elegance, and a deliberate
   convention change is a gate decision, never an implementation-time choice. Generic
   stack references (vue-style etc.) are NOT citations for this check. Same ≤3 cap.
