...
<!-- host:claude -->
|  | `/spec:verify [--codex] [--fix]` | dispatches the independent spec-verifier agent (four dimensions — Coherence includes the charter sub-audit, Reuse & Conformance covers project-idiom fit of new files); `--codex` adds codex as a heterogeneous second reviewer, `--fix` lets codex edit |
<!-- /host -->
<!-- host:codex -->
...
```
spec/
├── knowledge.md                      project knowledge INDEX — one line per subdoc, `- [kind/domain] file — hook` (format → references/knowledge-spec.md; legacy flat files read whole + declared)
├── knowledge/                        knowledge subdocs: domain fact files + long-form experience docs (frontmatter status; written by archive/ship/verify sediment steps, read two-step by every consumer)
├── changes/                          active change workspace
│   ├── .current                      current-change pointer (one line: change name) — gates target it first, fall back to the single-active rule when absent/dangling; written by research, switched by /spec:resume <name>, cleared by stash/archive of the pointed change; concurrent sessions on one project share it last-write-wins (same semantics as a git branch checkout)
│   └── <change-name>/
...
| knowledge base (project-level: knowledge.md index + knowledge/ subdocs — format → references/knowledge-spec.md) | Durable cross-change facts and rulings, routed by kind into domain subdocs / long-form docs; the index holds ONLY `- [kind/domain] file — hook` lines | anything change-specific → stays in that change's artifacts ｜ flat fact lines → subdocs, never the index ｜ a fact proven wrong is **replaced** (correction noted), never left contradicting | index one line per subdoc; facts one line each |
...
<!-- host:claude -->
The `check-gate.sh` hook checks the prerequisites before `/spec:apply` runs (proposal.md exists with all four sections + an unambiguous target: the current pointer or the single active change). It deliberately does **not** require the marker — apply appends the marker after the hook fires (requiring it there would deadlock the flow); the marker is enforced at archive time by `check-archive.sh`.
<!-- /host -->
<!-- host:codex -->
The `codex/hooks/check-gate` hook checks the prerequisites before `/spec:apply` runs (proposal.md exists with all four sections + an unambiguous target: the current pointer or the single active change). It deliberately does **not** require the marker — apply appends the marker after the hook fires (requiring it there would deadlock the flow); the marker is enforced at archive time by `codex/hooks/check-archive`. These hooks signal blocking by emitting `{"decision":"block"}` to stdout (not exit 2).
<!-- /host -->
...
<!-- host:claude -->
- Preference-type decisions **MUST** be put to the user via AskUserQuestion (`multiSelect: true` for pick-several decisions)
<!-- /host -->
<!-- host:codex -->
...
<!-- /host -->
- **Gate-aware next steps**: before recommending any `/spec:x` as the next step, pre-flight that command's gate preconditions yourself — they are all checkable (`## Open` empty; research.md/proposal.md present; **an unambiguous target: the current pointer set, or exactly one active change** — one `ls spec/changes/` tells you). A recommendation that will bounce off a gate steers the user into a wall you could see. When the target is ambiguous (>1 active, no pointer), the next step you present IS the remediation: the one-command fix is `/spec:resume <name>` (sets the pointer, everything else coexists untouched); alongside it, list the extras with their states — **flag verify-passed ones as archive candidates** (`/spec:archive` sediments their knowledge), offer `.paused` parking for anything the user wants visibly shelved (write the marker yourself, never the user's chore) — and only then name the target command.
...
<!-- host:claude -->
check-tbd / check-gate resolve their target by the **current pointer first** (`spec/changes/.current` naming an active change wins; multiple changes coexist freely under it), and `exit 2` only when **more than one active change exists AND no pointer selects one** (fix: `/spec:resume <name>` writes the pointer — or archive/stash the rest). **Dirs carrying `.paused`, or `quick.md`/`fix.md`/`loop.md` without proposal.md, don't count as active** (precedence: proposal.md wins — an upgraded tier dir is a normal full change); check-verify-reminder binds to the same pointer-or-single target. check-archive deliberately does **not** block on multiple changes (archiving is exactly how you get back down to one) and audits light-tier dirs by their own record (fix.md `status: shipped` + non-empty Audit; legacy quick.md `status: done` + non-empty Evidence). `/spec:ship` invocations are gated on preconditions only (a batch with entries exists) — never on the shipped status ship itself writes.
<!-- /host -->
<!-- host:codex -->
check-tbd / check-gate resolve their target by the **current pointer first** (`spec/changes/.current` naming an active change wins; multiple changes coexist freely under it), and block only when **more than one active change exists AND no pointer selects one** (fix: `/spec:resume <name>` writes the pointer — or archive/stash the rest). **Dirs carrying `.paused`, or `quick.md`/`fix.md`/`loop.md` without proposal.md, don't count as active** (precedence: proposal.md wins — an upgraded tier dir is a normal full change); check-verify-reminder binds to the same pointer-or-single target. check-archive deliberately does **not** block on multiple changes (archiving is exactly how you get back down to one) and audits light-tier dirs by their own record (fix.md `status: shipped` + non-empty Audit; legacy quick.md `status: done` + non-empty Evidence). `/spec:ship` invocations are gated on preconditions only (a batch with entries exists) — never on the shipped status ship itself writes.
<!-- /host -->
