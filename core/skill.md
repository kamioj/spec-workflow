...
<!-- /host -->
- **Gate-aware next steps**: before recommending any `/spec:x` as the next step, pre-flight that command's gate preconditions yourself — they are all checkable (`## Open` empty; research.md/proposal.md present; **exactly one active change** — one `ls spec/changes/` tells you). A recommendation that will bounce off a gate steers the user into a wall you could see. When a precondition fails, the next step you present IS the remediation, not the command name: for >1 active change, list the extras each with a recommended disposition — **stash is the zero-loss default** (artifacts stay warm; offer to write the `.paused` markers yourself — that is one file write per dir, never the user's chore), archive for those whose record shows them finished — and only after disposition name the target command.
