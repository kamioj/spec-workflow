...
<!-- host:claude -->
4. **`/spec:propose`** — write the four-section proposal.md, then **run the critique panel** (propose.md § Critique panel: auto default = necessity chief lens + regression-compat only (testability/performance are manual-pick lenses, available on request at the gate); one refutation round; findings → ledger round 0) (for major proposals, additionally add `--codex` for a heterogeneous critique)
<!-- /host -->
<!-- host:codex -->
...
<!-- host:claude -->
7. **Verification = apply's closing ledger round** — apply already dispatched the ONE independent spec-verifier pass and wrote the round; do **NOT** dispatch a second back-to-back full pass with zero code changed in between (that was a measured pure duplicate). This step **consumes** that round: open critical/major findings → fix → re-verify (scoped to the open findings + the fix diff, not a fresh full pass), **at most 2 auto-fix rounds** (aligned with the ledger's "open 2+ rounds forces fail" escalation); still-open items are NOT ground down further — they lead the acceptance report. For critical changes, `--codex` on `/spec:verify` still brings in Codex as a heterogeneous peer reviewer (`--fix` lets Codex apply fixes) — a genuinely different reviewer is not a duplicate
<!-- /host -->
<!-- host:codex -->
7. **Verification = apply's closing ledger round** — apply already spawned the ONE independent spec-verifier pass and wrote the round; do **NOT** spawn a second back-to-back full pass with zero code changed in between (that was a measured pure duplicate). This step **consumes** that round: open critical/major findings → fix → re-verify (scoped to the open findings + the fix diff, not a fresh full pass), **at most 2 auto-fix rounds** (aligned with the ledger's "open 2+ rounds forces fail" escalation); still-open items are NOT ground down further — they lead the acceptance report
<!-- /host -->
