...
<!-- host:codex -->
   - **Tool first**: Codex gates its structured question tool behind an experimental feature flag — with `default_mode_request_user_input` enabled, sessions carry a `request_user_input` tool. When that tool is available in this session, a preference-driven question MUST use it **when it fits the tool's shape: single-select AND at most 3 options** (the tool caps options per call). A question that doesn't fit — multi-select, or carrying 4 options — uses the plain-text form below instead, whatever the tool state; **never trim the mandatory skip/minimal candidate to squeeze a question into the tool**. When the tool is NOT available, say ONCE before the first question, then proceed in plain text:
