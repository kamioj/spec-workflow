...
2. **Check the slot**: if another change is currently ACTIVE (not paused, not quick-only), refuse — resuming would create two active changes and the gates would block both. Suggest pausing or archiving the occupant first.
