...
**Ledger format authority**: [`skills/core/references/loop-spec.md`](../skills/core/references/loop-spec.md) — loop.md structure, write ownership (loop.md model-only / .loop-state driver-only), field rules.
...
| `/spec:loop` | resume: a `paused`/fuse-stopped ledger exists → set `status: running`, re-bind the session, continue from the latest Retrospect's plan |
...
3. **Create the ledger**: `spec/changes/<kebab-name>/loop.md` per loop-spec — frontmatter (`status: running`, `max_rounds`, `no_progress_fuse: 3`, `started`), `## Acceptance` (checkboxes live ONLY here), empty `## Rounds` / `## Lessons`.
4. **Bind the session** (keeps other sessions in this project from being hijacked by the driver):
   ```
   printf 'session_id=%s\nrounds_injected=0\nretro_reinjects=0\nchecked_history=\ntree_fp_history=\n' "${CLAUDE_CODE_SESSION_ID:-}" > spec/changes/<name>/.loop-state
   ```
   This is the ONLY time anything other than the driver touches `.loop-state`.
