...
---
goal: <one-line goal>
status: running        # running | paused | done | aborted — the model writes this; the driver only reads it
max_rounds: 10         # plain integer; the round budget the user approved (primary safety mechanism)
no_progress_fuse: 3    # plain integer; consecutive no-progress rounds before the fuse blows
started: YYYY-MM-DD
---
# Loop: <change-name>
...
- **`status`**: `running` is the driver's arming condition — exactly one running loop.md per project, or the driver stands down. `paused` (user brake / stuck) and `aborted` (direction dropped) both release the stop immediately; `done` is set only after the final acceptance report.
- **`max_rounds` / `no_progress_fuse`**: plain integers only. Any non-integer (stray comment, quoted value, typo) makes the driver halt loudly with a "ledger corrupt" notice — deliberately, because a silently unparseable cap would otherwise mean an uncapped loop.
...
| Every turn end | `loop-driver` hook | decision table: corrupt? cap? acceptance met? retrospect written? progress? → re-inject or release |
