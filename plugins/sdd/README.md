...
> Spec-driven development 工作流：调研 → 拷问 → 提案 → HARD GATE → 实施 → 验证 → 交接 → 归档。

12 个独立 slash 命令 + 2 个硬约束 hook + 2 个开发 agent。每阶段可独立触发、可重入。
...
/sdd:verify
    ↓ 三维验证 OK
/sdd:handoff
    ↓ 生成 handoff.md，便于新会话接续
/sdd:archive
```

简单任务可 `/sdd:auto` 一键跑完。

## Context Handoff（防上下文溢出）

当会话变长、准备切项目、完成阶段性工作，或出现 `context_too_large` 时：

```
/sdd:handoff
```

它会生成 `spec/changes/<change-name>/handoff.md`，只保留目标、当前阶段、关键决策、未决问题、下一步、优先读取文件和最近验证结果。
...
原则：长期状态进文件，大日志 / 大 diff / 大 reference 只给路径和摘要，不粘全文。

## Micro Delegation（微任务委派）

如果运行环境支持子代理，并且用户明确授权：

```
用 sdd 做这个需求，允许子代理协助探索和验证。
```

主对话保留决策、HARD GATE、最终写入和整合；子代理优先承担容易撑大上下文的小任务：
...
子代理返回必须短：结论、证据路径、风险 / 未覆盖、下一步。代码修改型任务必须列 changed files。
...
- **子代理委派**：sdd plugin 在 research 阶段派全局 `@researcher`、在 apply 阶段派 plugin 内 `sdd-frontend-dev` / `sdd-backend-dev`；用户授权后，探索 / 归纳 / 验证类微任务也优先委派，主对话只整合短结论
- **并发**：互不依赖的任务一次性并发派单
...
| 命令粒度 | 12 个独立命令 | 4 命令一把梭 | skill-based |
