...
| Hook 脚本 | 触发命令 | 作用 |
|---|---|---|
| `hooks/check-tbd.ps1` | `/sdd:propose` 前 | research.md 含 `[TBD-N]` 则 `exit 2` 拒绝执行 |
| `hooks/check-gate.ps1` | `/sdd:apply` 前 | proposal.md 缺 `<!-- APPROVED: ... -->` 则 `exit 2` 拒绝执行 |

事件：`UserPromptSubmit`（hookify 同款，plugin wrapper 格式）。
脚本：`pwsh` Exec form（Windows 平台稳定）。
...
1. **Windows 优先**：hook 用 pwsh 写。跨平台需改 bash / sh 等价
2. **未做的扩展**：
