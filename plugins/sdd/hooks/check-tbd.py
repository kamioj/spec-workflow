#!/usr/bin/env python3
"""检查 /sdd:propose 前 research.md 是否仍有待决 TBD。"""
...
def main() -> int:
    ...
    if "/sdd:propose" not in user_prompt:
        return 0
    if not cwd:
        return 0
    ...
    if not changes:
        fail("SDD: 没有活跃 change。先调 /sdd:research <方向> 启动")
    ...
    for change in changes:
        ...
        if not research_path.is_file():
            fail(f"SDD: {change.name} 缺 research.md。先调 /sdd:research <方向>")
