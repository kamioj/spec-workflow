...
| 用户说 | 进入 |
|---|---|
| "按刚才聊的改 proposal" / "改 X 段" | `/spec:revise [section]` |
| "重新查 X" / "调研 Y" | `/spec:research <方向>` |
| "刚才聊出 N 个 TBD，问我" | `/spec:ask` |
| "需要画下架构图" | `/spec:design` |
...
- ❌ 给方案 A / B / C → 没问用户偏好就帮他选了（这是 /spec:ask 的事）
...
|---|---|---|---|
| `/spec:chat` | ❌ | ❌ | "只聊，先别动手" |
| `/spec:ask` | ❌ | ✅（TBD→Decided） | "结构化问我偏好型决策" |
| `/spec:revise [seg]` | ✅（局部） | ❌ | "我已经想清楚要改 X 段" |
| `/spec:propose` | ✅（整体） | ❌ | "从头写或重写" |
