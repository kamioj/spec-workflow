---
description: 实施代码，按 proposal/tasks 推进。命令前 hook 检查活跃 change / proposal.md 存在；apply 进入后自动追加 APPROVED 标记。增量验证：每节点做完就近调 /spec:verify，不攒到最后
...
3. **hook 校验**：`check-gate.sh` 仍在 `UserPromptSubmit` 时机检查 change / proposal 是否存在。`/spec:apply` 进入执行后自动追加 APPROVED，作为审计标记和后续流程状态。
...
| What 涉及 | 派单方式 |
|---|---|
| UI / 路由 / 组件 / 样式 / 客户端交互 | `spec-frontend-dev` |
| 服务端逻辑 / API / 数据模型 / DB 迁移 / 中间件 | `spec-backend-dev` |
...
2. **并发派单**（同一条消息发出两个 Agent 调用）：
   - `spec-backend-dev`：实现服务端，先返回符合契约的 mock 数据，再接真实数据源
   - `spec-frontend-dev`：实现客户端骨架，用 mock 数据 / TypeScript 类型对接契约
...
| flag | 启用项 | 效果 |
|---|---|---|
| `design` | anti-ai-slop | spec-frontend-dev 读 `skills/core/references/frontend-aesthetics.md` |
...
- 探索 / 归纳 / 验证类微任务优先委派给子代理，主对话不吞大日志 / 大 diff / 大范围搜索
- 每完成一个（或一组并行）节点 → 就近调 `/spec:verify`，**不攒到最后**
- 完成的任务在 tasks.md 标 `[x]` —— **谁完成谁标**：dev agent 标自己 owner 的子任务；主对话标自理项（如配置 / 脚本 / 跨模块协调类）

### 微任务委派触发

满足任一则优先委派（若运行环境支持且用户已授权子代理）：
- 需要读 3 个以上文件才能定位入口 / 调用链
- 测试 / lint / 构建输出超过 120 行
- diff 涉及 5 个以上文件，需要归纳影响面
- 需要对比多个相似实现
- 可以拆出互不重叠的文件范围给 worker 修改

子代理返回必须短：结论、证据路径、风险 / 未覆盖、下一步；不要返回完整日志。
