...
**Lens dispatch is automatic — there is no selection question, whoever invoked propose.** The user's control is opt-OUT, never a menu: `/spec:propose --skip <lens>` (repeatable — e.g. `--skip regression --skip performance`) excludes the named lens from this run; note each skip in the gate's Changes block (`panel: regression skipped by flag`) so the reduced coverage is on the record. Skipping every lens is legal (no panel; the gate still stands). An unknown lens name after `--skip` is flagged as a possible typo, never silently swallowed.
