...
- **0.5.2** — loop 验证改为终审模型（首次过夜 dogfood 实测：逐轮 verifier 派遣吃掉预算大头，一晚只推进两个小功能点）：
  ...
  - 同一"终审原则"落到 `/spec:apply`：节点级只做自跑工作检查，唯一的 spec-verifier 派遣在最后一项落地之后——旧 frontmatter 里"每节点就近调 /spec:verify"（实测中"验证-修改-验证-修改"空转的教唆源）已删除
